...
MLV205 needs more machinery than the other four together - "is this value still
holding a graph?" is a question asked backwards through detachments, scalar
helpers and guards - so its helpers live in `rules/loss_accum.py`. The rule
itself stays here, with its four siblings, where the registry and `spec.module`
already expect it.
"""
...
from .loss_accum import (_accumulated_label, _accumulations, _backwarded,
                         _consumed_downstream, _defined_outside,
                         _no_grad_region, _returned)
...
def loss_accumulated_with_graph(ctx) -> Iterable[Issue]:
    ...
    for relpath in sorted(ctx.modules):
        ...
        for name, loop, loc, scope, loss_ref, how in _accumulations(ctx, module):
            ...
            if _no_grad_region(loop):
                continue                # R18: no autograd graph here to keep alive
            if _consumed_downstream(module, name, scope):
                continue                # R18: the running total is the live value
