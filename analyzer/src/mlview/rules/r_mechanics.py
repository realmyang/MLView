...
from ..ir.model import CallSite, LoopIR, ModuleIR
from ..ir.symbols import dotted_text
from .helpers import calls_in_loop, literal_of, loop_chain, with_role, within_loop
from .mechanics_sites import (_anchor, _in_block, _producer,  # noqa: F401
                             _short, _static, _tail)
from .registry import rule
from .sched_cadence import (_BATCH_CADENCE, _EPOCH_CADENCE, _effective_loop,
                            _encloses_batch_loop, _sched_step_calls,
                            _scheduler_producer, _stepsize_uses_len,
                            _via_scheduler_name)
...
def _scaler_for(ctx, loop: LoopIR,
    ...
    R5, **identity**: a `scaler.step(optimizer)` in this very loop whose
    ...
    for call in body:
        ...
        if producer is not None and K.role_of(producer.fqn) == "GRAD_SCALER":
            return producer
        found = _scaler_behind(ctx, receiver)
        if found is not None:
            return found
    return None


def _scaler_behind(ctx, receiver) -> Optional[CallSite]:
    """The `GradScaler(...)` inside the factory that produced this receiver.

    `model, optimizer, scheduler, scaler = build(...)` is the other half of the
    same shape: the receiver's own producer is the **factory call**, and what
    says the value is a scaler is the return slot `ir.returns` already computed
    for that position - `via_fqns`. Reading it is not a guess; the construction
    is then located inside the callee so `_scaler_enabled` has a real
    `GradScaler(...)` to judge, and the walk gives up rather than inventing one.
    """
    entry = K.lookup_any(getattr(receiver, "via_fqns", ()) or ()) \
        if receiver is not None else None
    if entry is None or entry.get("role") != "GRAD_SCALER":
        return None
