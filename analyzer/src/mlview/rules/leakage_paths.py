"""Where is the split, and does this fit really run before it?

MLV101 and MLV102 both stand on one fact - "this `fit` sees data the split has
not separated yet" - and that fact is a path question, not a name question.
These helpers answer it: which call produced the value, which split consumed
it, whether the two are even in the same scope, and whether the name was
rebound in between (the commonest reason a fit that *looks* early is not).

`_note_refused_split` is the other half of the honesty rule: when the path
cannot be traced the rule stays silent and says so as a diagnostic, rather than
firing on a guess or going quiet without a word.
"""
...
# ---------------------------------------------------------------------------
# R5 - the three interprocedural walks
# ---------------------------------------------------------------------------
# Each one answers the same question as `_split_consuming` for a shape where the
# two halves of the defect are in different functions, and each pays its own
# provenance hop so the finding is de-rated and names the crossing. All three
# are `--dataflow ip` only: `local` is defined as the analysis that stops at the
# first `def`, and widening it there would be a second dialect rather than a
# narrower mode.
...
def _split_after_return(ctx, fit: CallSite, splits, name: Optional[str], ref):
...
def _fold_projection(ctx, fit: CallSite):
...
def _callee_fit_transform(ctx, cv_call: CallSite, data_name: str):
    ...
    if index is None and len(returned) > 1:
        ...
        # helper that hands back five values then matches all five, and the
        # walk picks the last `fit_transform` in the file rather than the one
        # that made this value: measured on
        # `analyzer/tests/accuracy/corpus/nlp_sklearn_text_leaky`, where it
        # blamed the test-set vectoriser at line 34 for the training matrix
        # built at line 29 - a high-confidence sentence that is simply false.
    ...
    for fit in ctx.calls_with_role("FIT_TRANSFORM"):
        if fit.function is not func or _stateless_fit(fit):
            continue
        targets = {fit.var}
...
def _stateless_fit(call: CallSite) -> bool:
