"""Units: the boxes a reader recognises as "a piece of this pipeline".

Every top-level function, every class, every module's entrypoint block and
every epoch/batch/fold loop becomes one unit node, parented so that a loop sits
inside the function that runs it. Framework hook methods (`training_step`,
`on_epoch_end`) are units too, and they carry their lane with them: the
framework declares what a hook is for, so `hook_stage` is written here and the
op vote in `core/build_ops.py` never overrides it.

`_owning_unit` is the question every later pass asks - "which unit does this
call belong to?" - and it answers with the innermost loop unit when there is
one, because that is where a reader looks for it.
"""
...
class UnitsMixin:
    """`GraphBuilder`'s unit pass. Mixed in, never instantiated."""
