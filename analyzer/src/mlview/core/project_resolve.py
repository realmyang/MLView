"""Step 1 of a scoped view: a parsed selector -> the nodes it anchors.

CONTRACTS 11.2, resolution half. Pure: no filesystem, no IR, no clock. Split
out of `core/project.py`, which owns the projection that consumes a
`ScopeResolution` and the catalogue that lists the selectors worth offering.
"""
...
#: `_with_descendants` keeps its underscore: `project.py` calls it by the name
#: it had before the split, and it is not part of the scoped-view API 11.6 names.
__all__ = ["ScopeResolution", "resolve_scope", "_with_descendants"]
