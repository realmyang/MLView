...
def build_loaders(args):
    ...
    # DEFECT: the validation split is read through the training pipeline, so
    # the reported IoU is measured on flipped, rotated, colour-jittered images.
    val_set = MultiTaskDataset(args.data, split="val", augment=train_augment())
...
def train_one_epoch(model, loader, criterion, optimizer, scheduler, device, args):
    ...
    for step, batch in enumerate(loader):
        ...
        # DEFECT: float16 autocast with no GradScaler anywhere. fp16 gradients
        # underflow to zero below ~6e-8, which is most of a segmentation
        # gradient, so the mask head trains on nothing and nothing raises.
        ...
        loss.backward()
        # DEFECT: zero_grad() now runs on EVERY batch rather than on the
        # accumulation boundary, so the gradients of the first accum-1 batches
        # are wiped before the step ever sees them - the accumulation is a
        # no-op and the effective batch size is 1/accum of what it says.
        optimizer.zero_grad(set_to_none=True)
        if (step + 1) % args.accum == 0:
            nn.utils.clip_grad_norm_(model.parameters(), max_norm=10.0)
            optimizer.step()
            scheduler.step()
        # DEFECT: the running total keeps the live tensor.
        running += losses["total"] * images.size(0)
        seen += images.size(0)
    return running / max(1, seen)


def evaluate(model, loader, criterion, device, args):
    # DEFECT: no model.eval(), so every BatchNorm2d in the shared encoder
    # updates its running statistics from the validation split.
    # DEFECT: no @torch.no_grad().
    total = 0.0
    intersection = 0.0
    union = 0.0
    correct = 0
    ...
    for batch in loader:
        ...
        # DEFECT: a second sigmoid on a head that already applies one, so the
        # 0.5 threshold now sits at sigmoid(0.5) = 0.62 of the real probability.
