"""Anchor generation, box encoding and the non-maximum-suppression pass.

Pure geometry: nothing here trains, and nothing here needs gradients.
"""
...
def postprocess(cls_logits: torch.Tensor, box_deltas: torch.Tensor,
    ...
    """Score -> threshold -> decode -> class-wise NMS, in that order."""
    scores, labels = cls_logits.sigmoid().max(dim=-1)
    keep = scores >= score_threshold
    scores = scores[keep]
    labels = labels[keep]
    boxes = decode_boxes(anchors[keep], box_deltas[keep])
    if boxes.numel() == 0:
