...
def eval_transform() -> transforms.Compose:
    # DEFECT: the evaluation pipeline now crops and flips at random, so every
    # number in the report is measured on a different view of the test set.
...
def build_loaders(args):
    ...
    # DEFECT: the test loader shuffles, so the Grad-CAM overlays written at
    # the end of the run are a different eight images every time.
...
def load_model(args, device):
    ...
    # DEFECT: neither map_location= nor weights_only=True.
    state = torch.load(args.checkpoint)
    model.load_state_dict(state["model"])
    # DEFECT: the model is returned in whatever mode it was constructed in.
...
def clean_accuracy(model, loader, device):
    # DEFECT: no model.eval(), so the ResNet's BatchNorms update their running
    # statistics from the test split while the headline accuracy is measured.
    # DEFECT: no @torch.no_grad(), so the whole test graph is built and dropped.
    correct = 0
    seen = 0
    for images, labels in loader:
        images = images.to(device, non_blocking=True)
...
def calibration_report(model, val_loader, test_loader, device):
    ...
    # DEFECT: the temperature is fitted on the TEST logits and the calibration
    # error is then reported on the same split, so eceAfter is a training
    # score dressed as a held-out one.
    scaler = TemperatureScaler().fit(test_logits, test_labels)
    with torch.no_grad():
