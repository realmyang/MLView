...
def rank(model, query_loader, gallery_loader, device):
    ...
    auc = float(roc_auc_score(matches.numpy(), scores))
    mean_ap = float(average_precision_score(matches.numpy(), scores))
    model.train()
    return rank1, auc, mean_ap
...
def main() -> None:
    ...
    for epoch in range(args.epochs):
        loss = train_one_epoch(model, criterion, train_loader, optimizer,
                               scheduler, device)
        rank1, auc, mean_ap = rank(model, query_loader, gallery_loader, device)
        print("epoch %d loss %.4f rank1 %.4f auc %.4f map %.4f"
              % (epoch, loss, rank1, auc, mean_ap))
        if rank1 > best:
            best = rank1
            torch.save({"model": model.state_dict(),
                        "head": criterion.state_dict(),
                        "epoch": epoch, "rank1": rank1},
