...
def flip_joints(joints: np.ndarray, visibility: np.ndarray, width: int):
    """Mirror the coordinates AND swap the left/right joint channels."""
    mirrored = joints.copy()
    mirrored[:, 0] = width - 1 - mirrored[:, 0]
    flipped_vis = visibility.copy()
    for left, right in FLIP_PAIRS:
        mirrored[[left, right]] = mirrored[[right, left]]
        flipped_vis[[left, right]] = flipped_vis[[right, left]]
    return mirrored, flipped_vis
