"""The distributed entrypoint (defective twin).

Defect 10: the one-cycle schedule is stepped once per epoch instead of once per
optimiser step, so the whole cycle is consumed in the first few epochs and the
rest of the run trains at the floor learning rate.
"""
    ...
    for epoch in range(CFG.epochs):
        ...
        if is_primary(rank):
            print("epoch %d train_loss %.4f val_top1 %.4f val_top5 %.4f"
                  % (epoch, float(train_loss), metrics["top1"], metrics["top5"]))
