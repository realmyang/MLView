"""Run configuration for the ImageNet-style classifier (defective twin).

Same knobs as the correct twin. The device is the first defect: it is pinned
to CUDA with nothing checking whether a CUDA device exists.
"""
...
def pick_device(local_rank: int = 0) -> torch.device:
    """Defect 1: the device is hard-coded, so a CPU box dies at the first .to()."""
