"""A small ResNet-shaped classifier plus the EMA (defective twin).

Defect 5: `forward` ends in a softmax and the loss is still CrossEntropyLoss,
so the log-softmax is applied twice and the gradient nearly vanishes.
"""
...
class ResNetish(nn.Module):
    """Stem, four stages of BasicBlocks, global pool, softmaxed head."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.softmax(self.head(self.dropout(x)))
...
def build_criterion() -> nn.Module:
    """Defect 5, the other half: cross entropy expects logits, not probabilities."""
