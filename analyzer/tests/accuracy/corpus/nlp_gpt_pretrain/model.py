"""A small decoder-only transformer, nanoGPT-shaped.

Three defects are planted in the module definition itself: a block that never
chains its base initialiser, a plain Python list of blocks, and a softmax in
front of a cross-entropy loss.
"""
...
class MiniGPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        for block in self.blocks:
            x = block(x)
        x = self.ln_f(x)
        logits = F.softmax(self.lm_head(x), dim=-1)
        loss = None
