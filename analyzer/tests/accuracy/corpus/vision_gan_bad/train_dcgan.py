"""DCGAN training (defective twin).

Defect 2: the generator's optimiser is never zeroed, so the generator update
is the running sum of every generator gradient in the epoch.
Defect 3: the real batch is never moved to the device the networks live on.
Defect 4: the running losses are accumulated as live tensors.
Defect 5: the training loader does not shuffle.
Defect 6 (no rule covers it yet): the discriminator is fed the *undetached*
fake batch, so every discriminator step also back-propagates into the
generator and the generator is updated by the discriminator's objective.
"""
...
def build_loader(batch_size: int = BATCH_SIZE, workers: int = WORKERS) -> DataLoader:
    """Defect 5: shuffle=False on the only training loader in the project."""
        ...
        for step, (real_images, _labels) in enumerate(loader):
            ...
            if step % 100 == 0:
                print("epoch %d step %d loss_d %.4f loss_g %.4f"
                      % (epoch, step, float(loss_d), float(loss_g)))
...
def sample_grid(netG, noise: torch.Tensor, epoch: int) -> str:
    """Defect 7: the sampler neither switches to eval nor disables autograd."""
    images = netG(noise)
