"""DDPM training: a small UNet epsilon-predictor, an EMA copy and a sampler.

The objective is `F.mse_loss(predicted_noise, noise)` - the model predicts the
noise that was added, so no activation is involved and there is no held-out
metric beyond the loss and the sample grid.
"""
...
class Ema:
    """A shadow copy of the weights, updated after every optimiser step."""
...
def train() -> nn.Module:
    ...
    for epoch in range(EPOCHS):
        ...
        print("epoch %d loss %.5f" % (epoch, running / max(batches, 1)))
        if epoch % 10 == 0:
            ...
            torch.save({"model": model.state_dict(),
                        "ema": ema.shadow.state_dict()}, "ddpm.pt")
    return model
