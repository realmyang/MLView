...
def eval_transform() -> transforms.Compose:
    # DEFECT: the evaluation pipeline kept the random flip and the random
    # erasing, so the query and gallery embeddings are computed from two
    # different random views and the similarity matrix is noise-dominated.
    return transforms.Compose([
        transforms.Resize(IMAGE_SIZE),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.ToTensor(),
