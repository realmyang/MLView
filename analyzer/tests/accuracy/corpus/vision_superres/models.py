"""ESRGAN pieces: an RRDB generator, a VGG-style relativistic discriminator and
a frozen VGG19 feature extractor for the perceptual term.

The feature extractor is the shape that reads like three defects and is none of
them: it is put in `eval()` once at construction, every parameter has
`requires_grad_(False)`, and it is then run **inside** the training loop on both
the super-resolved and the ground-truth image.
"""
...
class Discriminator(nn.Module):
    """Returns a raw score per image: the loss is BCEWithLogits."""
...
class VGGFeatures(nn.Module):
    """A frozen VGG19 truncated before the 5th max-pool, in eval mode forever."""
    ...
    def train(self, mode: bool = True):
        """Stay in eval mode whatever the parent module is told to do."""
...
def psnr(prediction: torch.Tensor, target: torch.Tensor,
    ...
    """A Python float, computed from a detached mean-squared error."""
    mse = F.mse_loss(prediction.detach(), target.detach()).item()
    if mse <= 0.0:
        return 99.0
    return float(10.0 * torch.log10(torch.tensor(max_value ** 2 / mse)).item())
