...
def val_transforms() -> Compose:
    """DEFECT: the validation pipeline is a copy of the training one, so the
    reported Dice is measured on randomly flipped, rotated and intensity-
    shifted patches that were cropped with a 1:1 positive/negative bias - a
    bias the real volumes do not have."""
    return Compose([
        LoadImaged(keys=KEYS),
        EnsureChannelFirstd(keys=KEYS),
        Orientationd(keys=KEYS, axcodes="RAS"),
