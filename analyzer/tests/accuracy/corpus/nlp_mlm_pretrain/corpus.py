...
The blocks are cut *after* the held-out slice is taken, so no document ever
straddles the train/validation boundary.
"""
...
def split_documents(documents):
    """Hold out whole documents before anything is tokenized."""
    return documents.train_test_split(test_size=VALIDATION_FRACTION, seed=SEED)


def encode(tokenizer, split, num_proc=4):
    def _encode(batch):
...
def prepare(checkpoint, data_files):
    ...
    encoded = encode(tokenizer, split)
    blocks = build_blocks(encoded)
    return tokenizer, blocks
