"""Single-node multi-GPU training launched with `torchrun`.

This is the *correct* twin of `infra_ddp_sampler_bug`. Everything a reviewer
would check is done: the sampler is reshuffled every epoch, the loss is
reduced across ranks before it is printed, evaluation runs under
`model.eval()` inside `torch.no_grad()`, batches are moved to the same device
the model lives on, and only rank 0 writes a checkpoint.
...
def setup_distributed() -> tuple[int, int, torch.device]:
    """Read the rendezvous environment `torchrun` exports."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="torchrun DDP reference")
    ...
    for epoch in range(args.epochs):
        # Without this the shuffle permutation is identical every epoch.
        ...
        scheduler.step()
        if rank == 0:
            print("epoch %d loss %.4f acc %.4f" % (epoch, loss, accuracy))
            if accuracy > best:
                best = accuracy
                os.makedirs(os.path.dirname(args.out), exist_ok=True)
                torch.save(ddp_model.module.state_dict(), args.out)
