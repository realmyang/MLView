"""The precision gate: five idiomatic, correct programs (CONTRACTS section 7.4).

`tests/clean/` is hand-written, correct ML code - the textbook loop, AMP with
gradient accumulation, a LightningModule, a HuggingFace `Trainer` run, and a
scikit-learn `Pipeline` + `GridSearchCV`. The bar A7 sets is **0 high and at
most 2 medium** across all of them, analyzed together *and* one at a time.
...
EXPECTED_FILES = ("amp_accumulation.py", "hf_trainer.py", "lightning_module.py",
                  "sklearn_pipeline.py", "vanilla_torch.py")
...
def test_the_corpus_is_the_five_files_the_contract_names():
...
def test_the_corpus_really_was_analyzed(corpus):
    ...
    assert {"torch", "sklearn", "lightning", "hf"} <= frameworks
