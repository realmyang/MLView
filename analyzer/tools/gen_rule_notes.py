"""The hand-written half of `docs/rules/<CODE>.md`: what each rule detects.

Data only - `gen_rule_docs.py` renders it. Split out of that script so the
generator stays readable: everything here is prose keyed by rule code, and
anything missing falls back to the `@rule` declaration text, so a new rule
still gets a usable page without touching this file.
"""
