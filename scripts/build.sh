...
# runs the same script through tools/sync-core.mjs, so a build that skipped this
# line would still package a current core - but `tools/verify.py --vsix` runs
# before step 4 in some flows, and the plugin's tracked copy is synced here too.
