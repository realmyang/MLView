...
| `doc_numbers.py` | The doc gate, checks 9-11 — the numbers a machine can settle: `docs/ACCURACY.md`'s headline against `analyzer/tests/accuracy/baseline.json`, an `upload-artifact` step whose hidden path would silently upload nothing — check 10 resolves `${{ env.NAME }}` against the workflow's own `env:` block, because PUB-01 wrote the same defect a second time with the dot-directory spelled through a variable and the check saw a path with no dot in it — and an "N steps" claim that is not the number of rows both e2e drivers print. |
...
| `test_check_docs.py` / `test_doc_numbers.py` / `test_doc_figures.py` / `test_doc_surfaces.py` / `test_vsix_check.py` | The gates' own test suite — ninety-nine cases: eighty-two throwaway trees, three unit tests (the symbol parser, the one spelling of `all` that counts as an advertisement, and the workflow-step block scoper), and fourteen that read the real repo. Running any one file runs all of them; `pytest scripts` does too. |
...
| 2 | Analyzer + rules | `python -m pytest analyzer/tests -q` | 2405 passed, 7 skipped on 3.12+ (the skips are the 3.10/3.11 `tomllib` split, in both directions: three that need a TOML parser and one that needs its *absence*). On 3.10 / 3.11 eight more skip: `analyzer/tests/fixtures/robustness/syntax/{pep695,fstrings}` are written in 3.12-only syntax and MLView parses with the host's own `ast`, so those hosts cannot read them at all — asserting `filesFailed == 0` there would be a claim about a file the host cannot open. Both read green on a 3.13 laptop and red on the matrix |
...
| 3 | Viewer tests | `npm test` in `webview` | 561 pass, 0 fail, 6 todo. The runner is `webview/tools/run-tests.mjs`, which **discovers** `test/*.test.mjs` (CONTRACTS 11.54 C1): the enumeration it replaced had gone three files stale, so three regression suites written in hardening round 1 ran green when invoked by hand and were invisible to `npm test` and to CI |
...
| 7 | Extension tests | `npm test` in `vscode-extension` | 401 pass, 0 fail |
...
| 8 | Plugin / MCP tests | `python -m pytest claude-plugin/tests -q -n auto` | 434 passed, 7 skipped in ~12 s with `-n auto` on this Mac (~34 s with `-n 4`, ~65 s without it) |
...
| 23 | Accuracy corpus (also a row in `scripts/e2e`) | `python tools/accuracy.py` | `accuracy gate: PASS` — **92 labelled programs, 312 labels**, precision **100.0% on every one of the 36 rules**, overall recall 72.4% / 66.7% visible / 64.5% high+medium, unseen recall **69.4%** raw / 63.0% visible / 60.2% high+medium, graph fidelity 85.0% (522 of 614); **zero `forbidden` and zero `unlabelled` findings**, nothing below `analyzer/tests/accuracy/baseline.json`. Re-recorded in hardening round 1 with `--allow-regression`, and the reason is in the baseline's own `note`: the corpus grew 15 → 92 programs and 78 → 312 labels, so the headline fell 73.1% → 72.4% while the **unseen** half rose 55.3% → 69.4%. Over the original fifteen programs alone this build reads 75.6% and the previous gate passes unchanged, which is the control that makes the re-record honest |
| 23c | Accuracy under `--dataflow ip` (DATAFLOW-IP) | `python tools/accuracy.py --dataflow ip` | `accuracy gate: PASS` against a **separate** ratchet, `analyzer/tests/accuracy/baseline.ip.json`, so the two modes cannot mask each other: precision **100.0%**, overall recall **76.3%** / 69.9% visible / 69.7% high+medium, unseen **73.7%** / 66.5% visible, graph fidelity 85.2%, zero `forbidden` findings, **+12 labels over `local`**. The one place `ip` is *narrower* than the mode it widens — an MLV110 on mlflow's MNIST autolog example that `local` reports and `ip` drops — is recorded `state: "open"` in `analyzer/tests/public_corpus/adjudication.json` rather than averaged into the row |
...
python scripts/test_check_docs.py         # the gates' own tests, all 99
