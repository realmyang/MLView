...
| 2 | Analyzer + rules | `python -m pytest analyzer/tests -q` | 1748 passed, 3 skipped (the third needs Python 3.10, where tomllib is absent) |
...
| 3 | Viewer tests | `npm test` in `webview` | 404 pass, 0 fail |
...
| 6 | Extension bundle | `npm run compile` in `vscode-extension` | `out/extension.js` 161.01 kb |
| 7 | Extension tests | `npm test` in `vscode-extension` | 308 pass, 0 fail |
...
| 8 | Plugin / MCP tests | `python -m pytest claude-plugin/tests -q -n auto` | 324 passed, 7 skipped in ~11 s (~42 s without `-n auto`) |
