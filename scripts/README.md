...
| 25 | CI matrix | `.github/workflows/ci.yml` | On the Sprint 4 wave 3 push to `sprint4` (run 34303572169): **12 jobs green, `smoke (macos)` skipped** — Python 3.10-3.13 (79-94 s each, now carrying the 500-file perf-budget corpus), Node 20/22 (126-131 s), `vscode-extension` 35 s, `claude-plugin` 57 s, `e2e (ubuntu, sh)` 290 s, `e2e (windows, powershell)` 499 s, `packaging (wheel + vsix)` 48 s and the accuracy corpus 16 s. **8m23s wall, ~41 billable minutes** (~23 ubuntu at 1x, per-job minute rounding + 18 windows at 2x + **0 macos**), and both e2e jobs archive `mlview-reports-*` (CI-ARTIFACTS-01). macOS runs on push to `main` and on pull requests, where it is 13 jobs |
