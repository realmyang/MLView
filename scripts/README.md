...
| `check_docs.py` | The doc gate, checks 1-8 and 12 (9-11 live in `doc_numbers.py`, 13-15 in `doc_figures.py`, 16 in `doc_claims.py`): dead paths, dead Markdown links, "known gap" bullets that still describe a failure somebody already fixed, gap bullets that cite nothing checkable or cite a symbol that has been renamed away, a frozen design record that has started reporting build state, a POSIX shell script written with CRLF, two docs that disagree about the size of the demo graph (check 8 now reads a two-line window and accepts `N nodes and M edges`, both of which a wrapped claim used to escape), and a roadmap item that `docs/STATUS.md` reports as shipped while `docs/ROADMAP.md` carries no `**Landed ...**` measurement note for it (check 12, PROC-01). |
...
| `doc_figures.py` | The doc gate, checks 13-15 — the figures the three *living* docs (`README.md`, `docs/STATUS.md`, `scripts/README.md`) keep requoting: `docs/STATUS.md`'s Components table against the newest `**Gates` paragraph of the same file and its `core/mlview` count against what `tools/sync-core.py` copies (REV5-05), any `N selectors` / `N error cases` / `N promoted counterexamples` claim against `contracts/scope.cases.json` — spelled-out numbers included, which is what "the ten-selector battery" was (REV5-08), and two documents naming two different runs as "the last full green push" (REV5-07). A dated record is exempt: check 8's `at the time` escape applies, and the bundled-core count is read out of table rows only. |
| `doc_claims.py` | The doc gate, check 16 — the one claim with no machine-readable copy anywhere: what the reader is told was verified, against what this repository's own record says was verified. `README.md` said *"every gate above is green on the build machine **and on the CI matrix**"* and `docs/STATUS.md` said the surfaces were *"exercised end to end ... across the CI matrix"*, while every job of every push on this branch came back *"The job was not started because recent account payments have failed"* (DOCS-CI-OVERCLAIM). A green claim and the matrix in one breath must say, in that same breath, whether the matrix ran; naming the block is the escape and the fix. Paragraphs, not lines — the STATUS claim was wrapped across two. |
...
suite, the accuracy corpus and the Linux end-to-end table — seven jobs, ~19
billable minutes. A **pull request, and a push to `main`,** adds Python 3.11 and
3.12, Node 22, the Windows end-to-end table, the macOS smoke job and packaging —
six more jobs and ~56 more minutes, of which 30 are the one macOS job (10x
weighting, each job rounded up to a whole minute on its own). Nothing reaches
`main` unchecked on any supported interpreter, either e2e driver or any of the
three platforms; the tiers move **when** the expensive half is paid for, not
whether. A push that touches only `**.md` and `docs/` runs nothing at all, and a
pull request deliberately has no such exemption, so a docs-only branch is still
checked before it merges. The two tiers never overlap: the cheap jobs keep the
same-repository double-billing guard and the expensive ones run exactly on the
events that guard excludes (see `.github/workflows/ci.yml`, whose header carries
the arithmetic, and the "Tests" section of the root README). There are three
...
| 2 | Analyzer + rules | `python -m pytest analyzer/tests -q` | 2229 passed, 4 skipped on 3.11+ (the skips are the 3.10/3.11 `tomllib` split, in both directions: three that need a TOML parser and one that needs its *absence*) |
...
| 2e | Interprocedural dataflow (DATAFLOW-IP) | `python -m pytest analyzer/tests/core/test_dataflow_ip.py -q` | 54 passed — **`ip` is the default** and `ir.build_ir.DEFAULT_DATAFLOW` is the one authority for that (CONTRACTS §3.11 R1.1), `local` is still reachable and still strictly narrower, the shipped sample is byte-identical in both modes; plus the ctor / return / method-arg / projection summaries in **both** modes, `local` silent where `ip` fires, the intersection over call sites (never the union), the `0.8 ** hops` de-rating asserted **numerically** (0.95 → 0.760 → 0.608 → 0.486), the `truncated` diagnostic a chain past the 3-hop cap must produce, and the "parameter merely named `X`" probe silent in both modes |
| 2e1 | Workspace objects and the knowledge tables (RECALL R2 / R3) | `python -m pytest analyzer/tests/core/test_workspace_ops.py analyzer/tests/core/test_knowledge_tables_r2.py -q` | 56 passed — the three call shapes that now draw a card (constructing a workspace class, invoking one, a factory's returned object), the five ways a script carries such an object, the honest `unknown` card for a factory nobody can follow, and the pandas / statsmodels / Prophet / `evaluate` / torchmetrics / Keras rows — every assertion run in **both** dataflow modes (CONTRACTS §5.3 A7–A12′, §14.2 F16–F21) |
| 2e2 | Value typing: LOGITS / PROBS / PREDS (RECALL R4) | `python -m pytest analyzer/tests/rules/test_value_typing.py -q` | 12 passed — MLV305 / MLV306 / MLV401 / MLV402 typed through a workspace helper's `return` and a `.detach().cpu().numpy()` tail, each with its **nearest** negative fixture, both directions in both modes, and the hop de-rating asserted numerically: a helper-traced MLV401 lands at 0.760 and can never reach `certain` (CONTRACTS §5.3 A13, §14.2 R19–R21) |
...
| 2p | Consolidation review findings (regression) | `python -m pytest analyzer/tests/rules/test_consolidate_regressions.py -q` | 15 passed — one test per confirmed review finding, each written to fail on the code as it was: MLV205 silent under `no_grad`, when the total is back-propagated one arithmetic step later and when the collected loss is returned — but **still firing** when the collected loss was already back-propagated in that scope; MLV101 and MLV103 reading a bare `return scaler.fit_transform(frame)` and agreeing with the `out = …; return out` spelling; a container crossing a `return` **and** a parameter (the GradScaler-in-a-parameter-dict) while two call sites with two different dicts carry nothing; MLV305 following a per-batch list through `np.concatenate` with the collector hop **intersecting** rather than unioning; and a pytest module that is not an evaluation region while a `test_*.py` that really trains still is |
...
| 3 | Viewer tests | `npm test` in `webview` | 538 pass, 0 fail |
...
| 7 | Extension tests | `npm test` in `vscode-extension` | 385 pass, 0 fail |
...
| 7e | Coverage-kind parity, host half (§2.6 C9) | `node --test test/coverage.test.js` in `vscode-extension` | 24 pass — the set this host reads compared **against `analyzer/src/mlview/core/coverage.py`'s own declaration** rather than a transcription of it, so a kind added to the core and not here reddens; plus `framework_filter` rendered with the analyzer's own sentence, its `count` (suppressed rule codes) never tallied as a blind site, and the chip that says `coverage: incomplete (framework filter)` when that is the only caveat |
| 8 | Plugin / MCP tests | `python -m pytest claude-plugin/tests -q -n auto` | 387 passed, 7 skipped in ~15 s with `-n auto` on this Mac (~34 s with `-n 4`, ~65 s without it) |
...
| 8f | Where the plugin writes (§13.2 C7) | `python -m pytest claude-plugin/tests/test_storage_paths.py -q` | 6 passed — `cache_dir()` resolves `MLVIEW_CACHE_DIR` → `<data dir>/cache` (named by `MLVIEW_DATA_DIR` **or** `CLAUDE_PLUGIN_DATA`, which is what a `claude --plugin-dir` session exports) → the core's own user cache, and **no branch is inside the analyzed project**; resolving a model-supplied `out` no longer creates `.mlview/` as a side effect of answering a containment question |
...
| 23 | Accuracy corpus (also a row in `scripts/e2e`) | `python tools/accuracy.py` | `accuracy gate: PASS` — 15 labelled programs, 78 labels, precision **100.0% on every one of the 36 rules**, overall recall **93.6%** / 82.0% visible, unseen recall **89.4%** raw / 70.2% visible, graph fidelity **99.4% (163 of 164)**; zero `forbidden` findings, zero unlabelled findings, nothing below `analyzer/tests/accuracy/baseline.ip.json`. **With no `--dataflow` this is now the `ip` run** (CONTRACTS §3.11 R1.1). Re-recorded 2026-09-15 for the recall wave (R1–R5); the baseline's own `note` says which change earned it, and `docs/ACCURACY.md` §8 is the record |
| 23c | Accuracy under `--dataflow local` (the opt-out) | `python tools/accuracy.py --dataflow local` | `accuracy gate: PASS` against a **separate** ratchet, `analyzer/tests/accuracy/baseline.json`, so the two modes cannot mask each other: precision **100.0%**, overall recall **82.0%** / 74.4% visible, unseen **70.2%** / 57.5% visible, graph fidelity **99.4%**, zero `forbidden` findings. `local` is deliberately the narrower analysis — MLV101 / MLV102 / MLV103's interprocedural paths are `ip`-only by construction (§3.11 R1.5) — so it sits **nine labels** below the default and no finding fires in `local` that does not also fire in `ip` |
...
| 24c | Per-file fact cache (CACHE) | `python -m pytest analyzer/tests/core/test_cache.py -q` | 35 passed — a warm run is **byte-identical** to `--no-cache`, an edited file is `cached: partial` and still byte-identical, a forged or foreign-identity sidecar is ignored rather than obeyed, and nothing is opened or written at all under `--relevance all` (which was the shipped default when this row was written and is now the opt-**out**: since Sprint 5 the default is `--relevance ml`, so the cache is on — and since C8 a plain `analyze .` writes **nothing into the analyzed folder**, the sidecar going to `$XDG_CACHE_HOME`/`~/Library/Caches`/`~/.cache`/`%LOCALAPPDATA%` under a directory named for the workspace path's hash, still overridable with `MLVIEW_CACHE_DIR`; four of the 35 pin that). The 31st stands a Windows in for this platform and asserts the MAC secret reads back **byte-identical** to what was written — without `os.O_BINARY` the ~12% of 32-byte keys containing an `0x0A` are CR-mangled on write and the cache is permanently, silently disabled |
...
| 26a | The **sdist** installs and runs | `python tools/wheel_check.py --sdist` | `wheel-check: OK mlview-0.1.0.tar.gz -> mlview 0.1.0, 4 node(s), 2 issue(s) in a clean venv` — the identical acceptance against `analyzer/dist/*.tar.gz`. `docs/VALIDATION.md` 4.1 runs `twine upload analyzer/dist/*`, which publishes **both**, and `pip install mlview` falls back to the sdist on any platform with no matching wheel; an sdist can omit package data a wheel carries, because a `MANIFEST.in` include is a separate mechanism from `[tool.setuptools.package-data]`. PyPI never lets a version be re-uploaded, so the fallback artifact is gated **before** the first upload. Not an `e2e` row — the wheel row is the one in the 20-step table; this is a Part-4 pre-flight (H11) |
| 27 | VSIX packages and stays small | `npm run package` in `vscode-extension`, then `python scripts/vsix_check.py` | `vsix-check: OK mlview-0.1.0.vsix: 172 files, 806.10 KB (78.7% of the 1 MB ceiling), 120 under extension/core/, 37 rule page(s), 0 bytecode entr(y/ies)` — re-measured after the **recall wave** (d7543b8), which added analyzer modules and moved all three counts from the Consolidate figures this row used to quote; `docs/STATUS.md`'s extension row carries the same 120. Expect them to have moved again by the time you read this — **the three numbers are an echo and only the ceiling is asserted**, which is why the row below says so. `npm run package` also *builds* the bundled core: where `npm run package` also *builds* the bundled core: `vsce package` runs `vscode:prepublish`, which runs `npm run compile`, which runs `node tools/sync-core.mjs` (C2). Packaged with no `--allow-missing-repository`. **The checker is the gate, not this row** (HOST-8): the file count and the KB have moved on every wave while hand-copied prose figures did not, so `scripts/vsix_check.py` re-derives all of them — the ceiling, `extension/core/` against `vscode-extension/core` on disk, `extension/docs/rules` against `docs/rules`, and zero `__pycache__` — and echoes what it measured. Both numbers rise with every analyzer module that lands, which is why the 1 MB ceiling is the assertion and the count is only ever an echo. CI runs it in the `packaging` job, and `scripts/test_vsix_check.py` negative-tests each rule against a synthetic package |
| 27a | The packaged VSIX RUNS its bundled analyzer (C2) | `python vscode-extension/tools/vsix_smoke.py` | `vsix-smoke: OK mlview-0.1.0.vsix: bundled core imports from the package, mlview 0.1.0 (schema 1.0), --demo byte-identical to contracts/graph.sample.json (46078 bytes), a live analysis drew 3 nodes / 2 finding(s)` — the row that makes an untracked `vscode-extension/core/` safe. Row 27 proves the package *contains* an analyzer; this one unzips the package, puts `extension/core` on PYTHONPATH the way `vscode-extension/src/bundledCore.ts` does, and from a working directory **outside this repository** imports `mlview` and asserts the module resolved inside the unzip (an installed copy on the same machine would otherwise answer and the run would prove nothing), checks its version against `vscode-extension/package.json`, emits `--demo` and compares it byte for byte with the frozen golden, and analyzes a real source file so the schema, the rule pack and the knowledge tables are exercised rather than replayed. CI's `packaging` job runs it immediately after row 27 |
...
| 25 | CI matrix, two tiers (C6) | `.github/workflows/ci.yml` | **A branch push runs 7 jobs for ~19 billable minutes** — analyzer on 3.10 and 3.13, webview on Node 20, `vscode-extension`, `claude-plugin`, the accuracy corpus and `e2e (ubuntu, sh)`. **A pull request, or a push to `main`, runs 13 for ~75** — those seven plus analyzer 3.11 / 3.12, webview Node 22, `e2e (windows, powershell)`, `smoke (macos)` and `packaging (wheel + vsix)`. Both figures are arithmetic over the per-job durations of the last full green push, the Sprint 5 review-fix integration push to `sprint5` (run 34454599867), which ran **12 jobs for ~44 billable minutes** under the single-tier matrix this replaces: `e2e (ubuntu, sh)` 6m29s → 7 min, py3.10 90 s → 2, py3.13 146 s → 3, Node 20 150 s → 3, `vscode-extension` 37 s → 1, `claude-plugin` 48 s → 1, accuracy 15 s → 1 — **19** — with GitHub's rule that **each job is rounded up to a whole minute on its own** before its runner weight (ubuntu 1x, windows 2x, macos 10x) is applied. The full tier adds py3.11 3 + py3.12 3 + Node 22 3 + packaging 1 ubuntu, `e2e (windows, powershell)` 7m53s → 8 x 2 = **16**, and `smoke (macos)` 2m30s → 3 x 10 = **30**: one macOS job costs more than a whole branch push. A push that touches only `**.md` and `docs/**` runs **nothing**; a pull request has no such exemption. **These two totals are still an estimate — the first push of the consolidation branch (run 34903408702, 2026-09-14) billed nothing, because GitHub refused to start any job on it: *"The job was not started because recent account payments have failed or your spending limit needs to be increased"*, the same annotation the `sprint5` run nine hours earlier carries, so the block is the account's and predates this branch. What that run DID settle is the split: it created exactly the **seven** cheap-tier jobs and marked all **five** full-tier jobs `skipped`, which is the behaviour this row claims. Re-measure the minutes from the first push after the account's Actions billing is restored (`gh run view <id> --json jobs`).** The split itself is not an estimate: the cheap tier keeps the same-repository double-billing guard and every full-tier job runs only on the events that guard excludes, so on a same-repo pull request the push event pays for one tier and the `pull_request` event for the other, each job exactly once. Both e2e jobs archive `mlview-reports-*` (CI-ARTIFACTS-01); `CHANGELOG.md` carries the Sprint 5 waves and the two matrix-only defects they caught (a Windows path separator asserted with `path.join`, and a `generatedAt` stripped at the wrong level of the document — neither reachable from a single-platform, single-second run, which is the argument for the matrix) |
...
holds the **built** `vscode-extension/core/mlview` to `analyzer/src/mlview` and
also fails when `.vscodeignore` would drop `core/` out of the package, or when
nobody has built it), the two
...
3. `tools/sync-core.py` → copies `analyzer/src/mlview` into **both**
   `claude-plugin/vendor/mlview` (tracked) and `vscode-extension/core/mlview`
   (a gitignored **build artifact** since C2 — `npm run compile`, `npm run
   pretest` and `vsce package`'s `vscode:prepublish` each run
   `vscode-extension/tools/sync-core.mjs`, which runs this same script, so the
   copy exists on every path that tests or packages the extension)
...
**Why one of those two copies is in git and the other is not (C2).**
`claude-plugin/vendor/mlview` stays **tracked**: `claude plugin install` copies
the plugin directory verbatim off a git ref, so for that host whatever git holds
is what the user runs, and an untracked vendor directory is a plugin with no
analyzer. It is temporary — the day the `mlview` wheel is on PyPI, `.mcp.json`
can depend on it and the directory goes. `vscode-extension/core/mlview` is the
opposite case: the VSIX is a build artifact that `vsce package` produces from a
working tree, so that copy only has to exist at package time. It is **gitignored**
and built, which removes 100-odd duplicated files from every diff, every review
and every rebase without weakening a single gate: `python tools/verify.py --vsix`
still byte-diffs the built copy against `analyzer/src/mlview` (and still refuses a
`.vscodeignore` that would drop it from the package),
`vscode-extension/test/core-untracked.test.js` asserts that `git ls-files` is
empty for it *and* that it was built anyway, and the `packaging` CI job runs the
analyzer out of the packaged VSIX (`vscode-extension/tools/vsix_smoke.py`).
...
17. `python scripts/test_check_docs.py` — the doc gate's own self-test (it also runs `test_doc_numbers.py`, `test_doc_figures.py`, `test_doc_claims.py` and `test_vsix_check.py`)
...
The consolidation wave added **check 16**, in `scripts/doc_claims.py`, for a rot the first fifteen cannot see:
not a number that drifted from a file in the tree, but a **claim with no copy anywhere**. `README.md` opened
"What is verified, and what is not" with *"Every gate above is green on the build machine **and on the CI
matrix**"*, and `docs/STATUS.md` said the listed surfaces were *"exercised end to end on this machine and, on
every push, **across the CI matrix**"* — while every job of every push on this branch came back *"The job was
not started because recent account payments have failed or your spending limit needs to be increased"*, so
nothing in the tree had run on Python 3.10, 3.11 or 3.12, on Windows or on macOS, and the five full-tier jobs
had never run at all (DOCS-CI-OVERCLAIM). `CHANGELOG.md` and row 25 above said so plainly; the two documents a
newcomer reads first did not. The rule is deliberately narrow — describe the matrix freely, and report a
**named** run as green (row 25's "the last full green push (run 34454599867)" is exactly what it must not
flag) — but a green claim and the matrix in one breath has to say in that same breath whether the matrix ran.

```sh
python scripts/check_docs.py              # the repo
python scripts/check_docs.py --root DIR   # any tree
python scripts/test_check_docs.py         # the gates' own tests, all 74
...
| `tools/sync-core.py [--check]` | The only writer of `claude-plugin/vendor/` **and `vscode-extension/core/`** — the two vendored analyzers that let the plugin and the VSIX run with no pip install. The first is tracked, the second is a gitignored build artifact (C2), so `--check` holds the tracked copy to `analyzer/src` unconditionally and reports an unbuilt copy as "not built" rather than as drift; `tools/verify.py --vsix`, which runs after a build, is the row that insists it exists. `vscode-extension/tools/sync-core.mjs` is the wrapper `npm run compile` / `pretest` / `vscode:prepublish` call, so no npm path can package a stale core. |
