...
| 3 | Viewer tests | `npm test` in `webview` | 583 pass, 0 fail, 3 todo — hardening round 2 closed three of round 1's six by widening the drag-pan guard to every canvas overlay (`HOSTS-UX-LEGENDPAN`); the three left are the legend's missing Escape rung and the Findings panel's clean state over a blind run, each named in its own `todo` string. The runner is `webview/tools/run-tests.mjs`, which **discovers** `test/*.test.mjs` (CONTRACTS 11.54 C1): the enumeration it replaced had gone three files stale, so three regression suites written in hardening round 1 ran green when invoked by hand and were invisible to `npm test` and to CI |
