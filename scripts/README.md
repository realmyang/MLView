...
| 2 | Analyzer + rules | `python -m pytest analyzer/tests -q` | 1722 passed, 3 skipped (the third needs Python 3.10, where tomllib is absent) |
...
| 3 | Viewer tests | `npm test` in `webview` | 384 pass, 0 fail |
...
| 7 | Extension tests | `npm test` in `vscode-extension` | 298 pass, 0 fail |
...
| 8 | Plugin / MCP tests | `python -m pytest claude-plugin/tests -q -n auto` | 305 passed, 5 skipped in ~11 s (~42 s without `-n auto`) |
...
| 27 | VSIX packages and stays small | `npm run package` in `vscode-extension` | `Packaged: mlview-0.1.0.vsix (126 files, 576.92 KB)` — no `--allow-missing-repository`, `core/mlview` (80 files: NB adds `ingest/notebook.py`) and the 37 rule pages included, under the 1 MB ceiling |
...
| 25 | CI matrix | `.github/workflows/ci.yml` | On the Sprint 4 wave 4 push to `sprint4` (run 34311137829): **12 jobs green, `smoke (macos)` skipped** — Python 3.10-3.13 (83-106 s each), Node 20/22 (124-140 s), `vscode-extension` 36 s, `claude-plugin` 56 s, `e2e (ubuntu, sh)` 252 s, `e2e (windows, powershell)` 396 s, `packaging (wheel + vsix)` 47 s and the accuracy corpus 14 s. **6m39s wall**, and both e2e jobs archive `mlview-reports-*` (CI-ARTIFACTS-01). macOS runs on push to `main` and on pull requests, where it is 13 jobs. Wave 4 took four fix iterations to get here, and three of the four were real: NB's two `.mlview.toml` tests needed the `NEEDS_TOMLLIB` mark that three other files already use, its byte-identity test was comparing wall clocks, and `core/cache._secret` was writing the MAC key in Windows TEXT mode — a shipped bug the randomly-ordered `test_cache.py` had been sampling as an intermittent red |
