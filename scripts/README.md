...
| 2 | Analyzer + rules | `python -m pytest analyzer/tests -q` | 2574 passed, 9 skipped and 24 xfailed on 3.12+ (two skips are the 3.10/3.11 `tomllib` split in both directions, two are the offline-HTML fallback a synced viewer bundle makes unreachable, four want public-corpus clones under `MLVIEW_PUBLIC_CORPUS_DIR` — set it and they run — and one is a rule probe that says in words what it could not resolve). On 3.10 / 3.11 eight more skip: `analyzer/tests/fixtures/robustness/syntax/{pep695,fstrings}` are written in 3.12-only syntax and MLView parses with the host's own `ast`, so those hosts cannot read them at all — asserting `filesFailed == 0` there would be a claim about a file the host cannot open. Both read green on a 3.13 laptop and red on the matrix |
