...
other than `auto` dropped rules the detected frameworks would have run. When any of
them appears, add one line after the table naming
the rules from that row's `codes` — quote them, do not guess — and never present the
result as a clean file; read the row's own `message` for what its `count` counts.
Prefer running on the containing directory and passing
