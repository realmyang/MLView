...
not be traced so the leakage rules could not check it, and `framework_filter` when
a `--framework` other than `auto` dropped rules — each with the analyzer's own
...
1. **Shape** — how many files were analyzed, which frameworks were detected, and
   ...
   *scope*, and each says so in its `note`; none of them is a statement about
   ...
   `framework_filter`): read the rule codes out of `coverage[].codes` and name them
   before you report the count, which that block makes a floor rather than a
   verdict. `count` there is the number of blind spots — sibling modules, untraced
   sites, or, for `framework_filter`, rules the filter dropped — so read the row's
   own `message` for what it counts rather than assuming.
