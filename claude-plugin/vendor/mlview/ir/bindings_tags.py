"""What a call hands back: the `ValueTag`s on a call's output.

`call_output_tags` is the whole of the question "the right-hand side is a call;
what does the name on the left now carry?" - the knowledge tables' output tags,
one level of in-workspace return-type inference (`ir/returns.slot_of`), the
pandas frame pass-through, the identity methods (`.to(...)`, `.cuda()`) that
hand their receiver straight back, and the estimator attributes
(`best_estimator_`) that hand back the model inside a search object.

The name regexes live here too, but only `_reinforce` may read them: a name
alone never establishes a tag, it only strengthens one dataflow already found
(CONTRACTS S6).
"""
...
__all__ = ["ESTIMATOR_ATTRS", "IDENTITY_METHODS", "TENSOR_ROLES",
...
#: GRAPH-R2. Roles whose value is the receiver's data in a new container, so
#: the receiver's tags travel through them. `FRAME_OP` is transparent (no node);
#: `FRAME_TEMPORAL` and `FRAME_RESHAPE` draw a node and carry the tags anyway.
_FRAME_CARRY_ROLES = frozenset({"FRAME_OP", "FRAME_TEMPORAL", "FRAME_RESHAPE"})
...
def call_output_tags(call: CallSite, scope: ScopeIR) -> Tuple[str, ...]:
    ...
    if role in _FRAME_CARRY_ROLES and receiver is not None:
        ...
        # GRAPH-R2 adds the windowing and regrouping families to the same
        # branch: `df["y"].shift(1)` and `df.groupby("k")` are still the same
        # data, so the tags travel; they differ from `FRAME_OP` only in that
        # they *draw a box*, which is a `TRANSPARENT_ROLES` question and not a
        # tag question.
