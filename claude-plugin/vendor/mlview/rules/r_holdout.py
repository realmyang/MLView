...
The five rules are here; what they read is three modules beside this one -
`rules/holdout_tables.py` (the metric sets and the four shared one-liners),
`rules/holdout_splits.py` (MLV106/114/121's chain and transform walks) and
`rules/holdout_scores.py` (MLV305/306's prediction argument and decision test).
The entry points stay in this file so `RuleSpec.module` and every `CONTRACTS`
citation keep naming `r_holdout`.
"""
...
from .holdout_scores import (_decided, _prediction_arg, _scored_value,
                             scored_value)
from .holdout_splits import (ORDERED_SPLITTERS, _HOLDOUT_PAIR,  # noqa: F401
                             _augmenting_factories, _augmenting_pipelines,
                             _eval_loaders, _holdout_of, _ordered_splitter,
                             _returned_pipeline, _subset_name,
                             _temporal_signals, _upstream, _workspace_dataset)
from .holdout_tables import (_CLASS_METRICS, _RANKING_METRICS,  # noqa: F401
                             _SCORE_METRICS, _anchor, _element_calls,
                             _short, _static)
        ...
        for call in module.calls:
            ...
            evidence = [
                ...
                ("dataflow_direct",
                 ...
                 % (name, producer.fqn or source), 1.0),
                ("knowledge_table",
