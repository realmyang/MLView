...
test('the demo SVG carries all 59 cards and all 51 edges (VIEW-07)', { skip: !demo && 'run scripts/e2e first — no .mlview/graph.json' }, async () => {
  ...
  // The demo is 59 nodes / 51 edges (CONTRACTS §14.1 and §17 E25: the five added
  // cards are the workspace objects §5.3 A7-A11 recover — only the picture moved,
  // all 15 findings kept their code, line, severity and confidence).
