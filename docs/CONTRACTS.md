...
| `--dataflow local\|ip` | `ir.build_ir.DEFAULT_DATAFLOW` (**`local`** in this build; the constant is the authority and this table follows it) | §3.11. |
...
**Acceptance (normative), on the 525-file synthetic** (uncapped 1972 nodes / 2273 edges / 126 findings): **`edges_lost == 0` and zero floating cards at every budget**, and `drawn + merged + absorbed == the uncapped edge count`, exactly. A *floating card* is a card with no edge, no parent and no children; **raw degree-0 is not the metric**, because a ghost has no edges by construction — the test asserts both that floating is empty **and** that every edgeless survivor is a ghost. Every finding resolves to a node at every budget and the finding **set** is identical to the uncapped one. `contracts/validate_sample.py` returns zero errors at 45, 100, 400 and 2000.
...
**The rule, absolute: the standalone report never navigates its own document.** No `.click()` on a same-frame anchor, no `location.href` assignment, no `location.assign` / `location.replace`, no `window.open`. A source scan of `webview/src/bridges.ts` gates all five. `deepLinkPlan({embedded, local, absFile, file, line, col})` is a pure, exported function of two booleans and is the only place the outcome is decided: `embedded` (either `local`) → `copy`; not embedded and not `local` → `copy`; not embedded and `local` → `launch`. `copy` puts `file:line` on the clipboard and toasts *"Copied `<file>:<line>` — open the report locally to jump into VS Code"* with the anchor below; `launch` appends a hidden `<iframe>` at the `vscode://` URL, removed after ~1500 ms, and the existing blur detection copies `file:line` and toasts if no blur arrives within 400 ms. **`embedded` is `window.self !== window.top`, read inside `try/catch`, and a throw counts as embedded**, since only a cross-origin embedding can make that comparison raise. `url` is built in both modes and is always `vscode://file/<absFile>:<line>:<col + 1>`.
...
| **D7** | A `launch` never accumulates frames: existing `iframe.mlv-deeplink` elements are removed before a new one is appended, so six clicks inside the 1500 ms window invoke the OS handler once, for the location last asked for. The 1500 ms delay is a lifetime, not the protection. |
...
| 11.40 | LM-tool scope and depth; multi-root folders; CFG-ONE host half; the gallery and walkthrough | **§12.6**, **§12.3**, **§12.5** |
...
*MLView Contracts v1.1 — 2026-09-14. Supersedes v1.0 + 63 amendments
