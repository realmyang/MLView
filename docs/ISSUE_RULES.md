...
**Detection.** A `DataLoader(...)` whose `dataset` argument's `ValueRef` carries `VAL_SPLIT`/`TEST_SPLIT` (or whose target name matches `(?i)(val|valid|test|eval)_?(loader|dl)` — reinforcing only) with the literal `shuffle=True`.
...
**Detection.** An `AugAssign` (`total += x`) or `x = x + y` inside a loop, where the accumulated `ValueRef` carries `LOSS` (or any tensor tag), the accumulator is defined **outside** the loop, and the right-hand side is not wrapped in `.item()`, `.detach()` or `float(...)` and is not inside a `torch.no_grad()` scope. Also fires for `losses.append(loss)` on a list defined outside the loop.
