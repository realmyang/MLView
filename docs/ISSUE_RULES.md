...
**Detection.** Find call sites whose resolved FQN carries the knowledge-table role `FIT` or `FIT_TRANSFORM` (`sklearn.preprocessing.*`, `sklearn.decomposition.*`, `sklearn.impute.*`, `sklearn.feature_selection.*`, `sklearn.feature_extraction.*`) whose primary argument's `ValueRef` carries `RAW_DATA` or `FEATURES` but **not** `TRAIN_SPLIT`. Then walk forward through the SSA-lite chain in the same scope: does a later statement call a `SPLIT`-role FQN (`train_test_split`, `KFold.split`, `StratifiedKFold.split`, `random_split`) consuming a value **reachable from the fit's output or its input**? Reachability is by value identity, so both `X = scaler.fit_transform(X); … train_test_split(X, y)` and `Xs = scaler.fit_transform(X); … train_test_split(Xs, y)` fire. Primary `loc` at the fit site; `relatedLocs[role="split_site"]` at the split.

**Through a return (R1/R5, `--dataflow ip`).** When no split is written in the fit's own scope, one further path applies: if the fitted value is **returned** and a caller hands the returned *position* straight to a split, that is the same leak with a `def` in the middle — `build_matrix()` scales the whole series, `walk_forward()` folds it. The match is on the split argument's own binding: its producer must be the very call to the fit's function, at a returned position the fitted value feeds. No name is matched across scopes, which is what REV5-01 forbids. The claim costs one `IP_HOP_WEIGHT` (0.95 × 0.8 = 0.76), so it reads `likely`, names the hop in its evidence and carries a `relatedLocs` entry at the call site.

**False-positive notes (each is a `_good.py`).** (a) The split is on a *different* dataset — guarded by requiring value identity through the SSA chain, never name equality. (b) Unsupervised code with no test set — guarded by requiring a split site to exist at all. (c) The fitted value carries only `TARGET` (e.g. `LabelEncoder` on `y`), which is usually benign — de-rate to `medium` with confidence ×0.6. (d) The transformer is stateless (`FunctionTransformer` with no fitted state) — listed in `knowledge/sklearn.yaml:stateless_transformers` and skipped.
...
**Detection.** Call sites with role `FIT` / `FIT_TRANSFORM` / `PARTIAL_FIT` whose primary argument's `ValueRef` carries `VAL_SPLIT` or `TEST_SPLIT`. Split tags are assigned at `train_test_split` by the known positional convention (`X_train, X_test, y_train, y_test`) and reinforced — never created — by `(?i)^(x|y)?_?(test|val|valid|eval|holdout)`. Fires regardless of statement order.

**Through a fold index (R5, `--dataflow ip`).** The argument may also be a fold rather than a named split: `fold_scaler.fit_transform(features[test_idx])`, where `test_idx` is the **second** name unpacked from the header of a `fold` loop over a `SPLIT`-role `splitter.split(...)`. Position 1 of that tuple is the held-out half by the scikit-learn splitter protocol — a fact about the library, not a guess about the variable's name. The projection costs one hop.

**False-positive notes.** (a) Transductive / semi-supervised setups that intentionally fit on unlabeled test features — skipped when the module imports `sklearn.semi_supervised.*`; otherwise suppressible with the ignore comment, and this is documented in the rule doc. (b) A regex-only match (no dataflow tag) is emitted at confidence ×0.8, which usually lands it in `likely` rather than `certain`.
...
**Through a helper (R5, `--dataflow ip`).** The fit may be one `def` away. The CV's `X` is resolved to the workspace call that produced it, the callee's `return` is read at the tuple position the caller unpacked, and a non-stateless `fit_transform` whose output reaches that position is the fit whose statistics every fold now shares. The crossing costs one `IP_HOP_WEIGHT` and names itself in the evidence.

**False-positive notes.** (a) Fold-invariant, stateless transforms — the `stateless_transformers` list is consulted and those are skipped. (b) If `MLV101` already fired on the same transformer at the same `loc` **of the same file**, `MLV103` is suppressed — never stack two findings for one root cause. The file qualifier matters now that the fit may live in another module: a reader of the training script who is told nothing because `features.py` carries an MLV101 has been told nothing about the folds, which is the only thing MLV103 exists to say.
...
**Detection.** A `DataLoader(...)` whose `dataset` argument's `ValueRef` carries `VAL_SPLIT`/`TEST_SPLIT` (or whose target name matches `(?i)(val|valid|test|eval)_?(loader|dl)` — reinforcing only) with the literal `shuffle=True`. A third form covers the **split dictionary**: `DataLoader(encoded["test"], shuffle=True)`, where the subscript base is a value the analyzer already traced to data and the key is a literal naming the held-out half. `dotted_text` answers `None` for a subscript, so that spelling — the HuggingFace `DatasetDict` one — used to have no name, no tags and no finding; it is weighed as name evidence (×0.8), never as dataflow evidence.
...
**Detection.** Within one loop body, compare statement indices of the `OPT_STEP`-role call and the `.backward()` call on the same iteration path (same immediate parent block, no intervening loop). Fire when `step_index < backward_index`. Also fire the variant `backward → zero_grad → step`, where the intervening `zero_grad` wipes the gradients that were just computed.
...
**Detection.** An `AugAssign` (`total += x`) or `x = x + y` inside a loop, where the accumulated `ValueRef` carries `LOSS` (or any tensor tag), the accumulator is defined **outside the accumulating loop**, and the right-hand side is not wrapped in `.item()`, `.detach()` or `float(...)` and is not inside a `torch.no_grad()` scope. Also fires for `losses.append(loss)` on a list defined outside the loop.

"Outside the accumulating loop" is the innermost loop that does the adding, not the outermost loop in sight: `total = 0.0` at the top of the **epoch** loop with `total += loss` in the batch loop inside it holds a whole epoch of autograd graphs, which is exactly the leak this rule exists to report. The `LOSS` tag is also read through one arithmetic step, because `loss = criterion(...) / ACCUM_STEPS` is how every gradient-accumulation tutorial is written and a division does not take a tensor off the graph; a value the author already took off the graph (`.item()`, `.detach()`, `float()`) stops that read.

**False-positive notes.** (a) Intentional multi-step loss accumulation for a single `backward()` — suppressed when `.backward()` is called on the accumulator itself. (b) A list of tensors later `torch.stack`ed and backwarded — same suppression.
...
**Detection.** Identify eval regions: (a) a `for` over a `LOADER`-tagged value whose body performs a model forward but contains no `.backward()` / `OPT_STEP`; or (b) a `FunctionDef` named `(?i)^(validate|evaluate|val|test|predict|inference)` whose body matches that shape. Fire when no `torch.nn.Module.eval()` call on the same `MODEL` `ValueRef` **dominates** the loop — searched in the enclosing function and, one level up, at the resolved call site.
...
**False-positive notes.** (a) `eval()` called once in an outer setup function — mitigated by the one-level dominance search; if the model is a parameter of the eval function and `eval()` is called at the single resolved call site, suppress. (b) Framework gate (Lightning `validation_step`, HF `Trainer`) suppresses entirely.
...
**Detection.** The value passed as the first (input) argument to a `torch.nn.CrossEntropyLoss` instance call, or to `torch.nn.functional.cross_entropy`, is traced through the SSA chain. Fire if it reaches `torch.softmax` / `F.softmax` / `Tensor.softmax` / `F.log_softmax`, **or** the output of a model whose resolved `forward` returns an expression ending in `nn.Softmax` / `nn.LogSoftmax`, **or** whose final `nn.Sequential` element is one of those, **or** — when no model class resolves — a workspace **helper** whose `return` is the softmax.

The model class is tested with `is_model_module`, not `is_nn_module`: a `pl.LightningModule` subclasses `nn.Module`, so `F.cross_entropy(self(features), labels)` in a `training_step` is a model forward in every sense this rule cares about, and the narrower test answered `False` for it. The helper path is tried **after** the model class, never before — a model's `forward` is a workspace function too, and pre-empting the class path would cost the finding its model class, its `model → loss` edge and its `definition` location. Crossing into a helper costs one `IP_HOP_WEIGHT` (0.95 × 0.8 = 0.76), so a helper-traced pairing is `likely` and never `certain`.

**One root cause, one finding.** A `LightningModule` applies the same `forward` from `training_step` *and* `validation_step`; a hand-written loop calls the same criterion in train and in eval. The finding is keyed on the offending softmax plus the model class it lives in, and the survivor is the first loss site in source order — two different models that each softmax before a CE still get one finding each.
...
Reported at the loss construction with a `relatedLocs[role="final_layer"]` at the model's last op. The chain reads the model class first (`is_model_module`, so a `LightningModule` counts) and then one workspace **helper** whose `return` is the squashing op, at the same one-hop cost as MLV401. De-duplication is by `(final op, variant)`: the same head feeding `BCEWithLogitsLoss` in one place and `BCELoss` in another is two different mistakes, not one reported twice.

**False-positive notes.** A custom module named `Sigmoid` that is not `nn.Sigmoid` — prevented by FQN resolution. An unresolvable producer drops the finding to `possible`.
...
**Detection.** For each `ClassDef` whose resolved base chain includes `torch.nn.Module` or `pytorch_lightning.LightningModule` / `lightning.LightningModule`, if it defines `__init__` and no statement in that body is a call to `super().__init__` or `<Base>.__init__(self, …)`, fire at the `def __init__` line with a `relatedLocs[role="definition"]` at the `class` line.
...
- **MLV114** *(medium)* — A `transforms.Compose([...])` / `albumentations.Compose` containing a `random_augmentation`-set FQN whose resulting value flows into a Dataset consumed by a `VAL_SPLIT`/`TEST_SPLIT` loader. FP guard: deliberate TTA — ×0.5 when the eval code averages over repeated passes or an identifier contains `tta`.
...
- **MLV207** *(medium)* — A `SCHED_STEP` call whose receiver's constructor is in the epoch-based set (`StepLR`, `MultiStepLR`, `ExponentialLR`, `CosineAnnealingLR`, `ReduceLROnPlateau`) but whose enclosing loop is the **batch** loop; and the inverse for the per-batch set (`OneCycleLR`, `CyclicLR`, `get_linear_schedule_with_warmup`) stepped in the epoch loop. Loop role comes from confirmed nesting, not names. FP guard: `StepLR(step_size=len(loader)*k)` — detect `len(<loader>)` in the argument and suppress. Second variant: `ReduceLROnPlateau.step()` called with no metric argument.
...
- **MLV305** *(medium)* — A metric call (`accuracy_score`, `f1_score`, `precision_score`, `recall_score`, `torchmetrics.functional.accuracy`) whose prediction argument carries `LOGITS`/`PROBS` and was not produced by `argmax` / `round` / a threshold comparison / `topk`. **Score-metrics carve-out**: `roc_auc_score`, `average_precision_score`, `log_loss` legitimately take scores and are in a separate list that never fires. The prediction argument is typed by `rules/valuetype.py` (below), so `accuracy_score(y, probabilities(model, x).detach().cpu().numpy())` is read for what it is.
- **MLV306** *(low)* — `roc_auc_score` / `average_precision_score` whose score argument carries `PREDS` and was produced by a `PREDICT`- **or `ARGMAX`-role** call, rather than by `predict_proba(...)[:, 1]` / `decision_function(...)`. A hard label is a hard label however the decision was taken, and `probs.argmax(axis=1)` is the commoner spelling in a torch program.
...
**`rules/valuetype.py` — one answer to "what is this value?" (R4).** Four rules ask the same question about the same kind of value: MLV305 (is this a class prediction or still a raw score?), MLV306 (is this a score or a decision already taken?), MLV401 (did this come from a softmax?) and MLV402 (did this come from a sigmoid?). The knowledge tables answer it for a *direct* call — `F.softmax` is `PROBS`, `torch.argmax` is `PREDS`, `nn.Module.__call__` is `LOGITS`, `predict_proba` is `PROBS`, `predict` is `PREDS` — and dropped it at the two hops a real program puts in the way: the `return` of a workspace helper, and the `.detach().cpu().numpy()` tail every torch program writes before handing a tensor to sklearn. `value_tags` reads the shortest answer first (the table, then the binding's own tags, then the pass-through receiver, then the callee's `return`, capped at three hops) and `returned_call_with_role` finds the op a callee hands back. It mints no tag of its own: every tag it returns was put there by a knowledge-table entry or by `ir.returns`, and the only thing it adds is the willingness to keep looking. A value it had to fetch out of a callee carries a `return` hop, so the finding that used it pays `IP_HOP_WEIGHT` once — 11.36 G6 applies to a rule that starts making cross-object claims whichever mode it is running in.
