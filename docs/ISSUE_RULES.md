...
**R5 (`--dataflow ip` only).** A second shape is reached when the fit and the split are in **different functions** and the value crossed between them through a `return` — `build_matrix()` scales the series, `walk_forward()` folds it. No name is matched across scopes: the split's argument must be bound by the very call site that invoked the fit's function, at a returned position the fitted value feeds, so the claim rests on the call graph rather than on a spelling. A returned *call* counts as the names it is written on, so `return scaler.fit_transform(frame)` reads exactly as the two-statement spelling. The finding pays one `IP_HOP_WEIGHT` per hop **once for the whole chain**, names the chain in its evidence, and can never reach `certain` — measured at 0.608 / `possible` on the two-module fixture in `analyzer/tests/core/test_review_fixes.py`.
...
**R5 (`--dataflow ip` only).** The fit may be one `def` away — `features, target = build_matrix(path)` then `cross_val_score(est, features, target)` is the commonest spelling of this defect in a research repository. The CV's `X` is resolved to the workspace call that produced it, the callee's `return` is read **at the tuple position the caller unpacked**, and a non-stateless `fit_transform` whose output reaches that returned value is the fit whose statistics every fold shares. When the caller's binding does not record which position it unpacked — the commonest reason being that the name is itself a parameter — only a returned value of the **same name** is accepted; a helper that hands back five values does not match on all five. That guard is not theoretical: without it the walk blamed the test-set vectoriser at `nlp_sklearn_text_leaky/sentiment.py:34` for the training matrix built at line 29.

**False-positive notes.** (a) Fold-invariant, stateless transforms — the `stateless_transformers` list is consulted and those are skipped. (b) If `MLV101` already fired on the same transformer at the same `loc` **and in the same file**, `MLV103` is suppressed — never stack two findings for one root cause. The file clause is R5's: MLV101 is anchored on the fit site and MLV103 on the cross-validation call, so now that the fit may be a `def` and a file away, "the same statement" has to mean the same file. A reader of `train.py` told nothing because `features.py` carries an MLV101 has been told nothing about the folds, which is the only thing MLV103 exists to say.
...
**R18 guards, kept without the widening.** Two further suppressions, both of which can only remove a finding: (f) the accumulating loop runs where autograd builds **no graph** — inside `with torch.no_grad():` or under a `@torch.no_grad()` decorator — so `running_vloss += loss_fn(model(x), y)` in a validation loop keeps nothing alive and the finding would simply be wrong; (g) the running total is **used as a live tensor** one arithmetic step later in its own scope — `style_loss += mse(...)`, `total = content + style_loss`, `total.backward()` — where `.item()` would detach the term from training. R18's other half, reading a LOSS tag through `loss = criterion(...) / ACCUM_STEPS`, is deliberately **not** shipped: it was measured against the pinned public corpus and produced three false positives there, and a rule that accuses correct code costs more than a rule that misses a defect.
...
**Detection.** The value passed as the first (input) argument to a `torch.nn.CrossEntropyLoss` instance call, or to `torch.nn.functional.cross_entropy`, is traced through the SSA chain. Fire if it reaches `torch.softmax` / `F.softmax` / `Tensor.softmax` / `F.log_softmax`, **or** the output of a model whose resolved `forward` returns an expression ending in `nn.Softmax` / `nn.LogSoftmax`, **or** whose final `nn.Sequential` element is one of those. The model class is any **model module** (`ClassIR.is_model_module`), so a `pl.LightningModule`'s `self(features)` inside a `training_step` is a forward pass here exactly as a bare `nn.Module`'s is — which is what made the whole Lightning half of this rule reachable. Failing all of those, and **only after** the model-class branch (a model's `forward` is a workspace function too, and pre-empting the class path would cost the finding its model class, its `model → loss` edge and its `definition` location), R4 reads one workspace helper's `return`: `def probabilities(m, x): return F.softmax(m(x), dim=-1)` pairs just as wrongly as an inline softmax and pays one `IP_HOP_WEIGHT` for the crossing, landing at `likely` rather than `certain`.

**False-positive notes.** (a) An intervening `log()` making it effectively log-softmax — the chain walker detects it and redirects to `MLV403` (later-tier) rather than firing here. (b) A softmax applied conditionally (`if self.return_probs:`) — confidence ×0.6 when the softmax sits inside an `If`. (c) A user class *named* `Softmax` that is not `nn.Softmax` — prevented by FQN resolution rather than name matching. (d) **The same softmax reported twice.** One root cause is one finding and one edit: a LightningModule applies one `forward` in both `training_step` and `validation_step`, and a hand-written loop calls one criterion in train and in eval, so the second loss site merges into the first finding's `relatedLocs` instead of raising a copy. Measured on `analyzer/tests/accuracy/corpus/lightning_tabular`, where the second copy was a false positive.
...
**Detection.** Two symmetric checks using the same final-op chain walk as MLV401. The walk steps through shape-only tensor methods (`.squeeze(-1)`, `.view(...)`), reads both operands of a `BinOp` argument (a pairwise ranking loss is `criterion(pos - neg, target)`), resolves a forward through any **model module** rather than only a bare `nn.Module`, and — R4 — through one workspace helper's `return`, which pays an `IP_HOP_WEIGHT`.
...
- **MLV208** *(medium)* — With a `GradScaler` present, require `scaler.scale(loss).backward()`, `scaler.step(optimizer)`, `scaler.update()`. Sub-variants: a bare `loss.backward()` while a scaler exists; `step` without `update`; `optimizer.step()` instead of `scaler.step(optimizer)`; `clip_grad_norm_` without a preceding `scaler.unscale_(optimizer)`. FP guard: `GradScaler(enabled=False)`. **Which scaler governs the loop** is answered proximity-first and then by identity (R5): the `GradScaler(...)` in the loop's own function, failing which a `scaler.step(...)` **in this very loop** whose receiver resolves to a `GradScaler(...)` written anywhere else — the shape a `make_state()` factory or a parameter dict writes. The first arm guesses from position; the second one knows, so it is strictly better evidence and is taken only when the first finds nothing.
...
- **MLV305** *(medium)* — A metric call (`accuracy_score`, `f1_score`, `precision_score`, `recall_score`, `torchmetrics.functional.accuracy`) whose prediction argument carries `LOGITS`/`PROBS` and was not produced by `argmax` / `round` / a threshold comparison / `topk`. **Score-metrics carve-out**: `roc_auc_score`, `average_precision_score`, `log_loss` legitimately take scores and are in a separate list that never fires. **R4 value typing** (`rules/valuetype.py`) answers *what this value holds* for MLV305/306/401/402 in one place, and follows two hops the knowledge tables alone could not: the `.detach().cpu().numpy()` tail, which changes the container and not the content, and one workspace helper whose `return` is the softmax / argmax. A per-batch list joined by `np.concatenate(predictions)` is typed by the **intersection** of every `append`, so one unreadable append yields no answer rather than a union that would make a correct program look wrong; a tag fetched out of a callee carries a `return` hop and is de-rated for it.
- **MLV306** *(low)* — `roc_auc_score` / `average_precision_score` whose score argument carries `PREDS` and was produced by a `PREDICT` **or an `ARGMAX`** role — `<clf>.predict(...)`, and since R4 also `logits.argmax(dim=1)`, the torch spelling of the same mistake — rather than `predict_proba(...)[:, 1]` / `decision_function(...)`. The value typing follows the same two hops MLV305 reads, and the message names whichever producer it found.
