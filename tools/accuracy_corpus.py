...
def _default_dataflow() -> str:
    """The product default, read from the one place R1 says owns it.

    REC-06: this module used to spell `"local"` again in `run_corpus`'s
    signature, which is exactly the drift R1 exists to prevent - the CLI scored
    `ip` while `pytest analyzer/tests/accuracy` scored `local` and both called
    themselves "the default". Read lazily, because the analyzer is not
    importable until `_import_analyzer` has fixed `sys.path`.
    """
    _import_analyzer()
    from mlview.api import DEFAULT_DATAFLOW
...
def run_corpus(corpus_dir: str = CORPUS_DIR, only: Sequence[str] = (),
    ...
    `DEFAULT_DATAFLOW` - the single authority R1 names. Each mode is scored
    against its own baseline file, never against the other's; a caller that
    wants the non-default ratchet names its mode, and the report says which
    mode it scored either way."""
