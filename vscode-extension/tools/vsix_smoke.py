...
that matters now that the bundled core is no longer a tracked directory (C2).
The VSIX is built from a working tree by `vsce package` after
...
PYTHONPATH exactly as `vscode-extension/src/coreClient.ts` does for a bundled
core, and from a cwd *outside* this repository
  ...
  4. analyze a real (tiny) source file, so the schema, the rule pack and the
     knowledge tables are exercised rather than replayed from a fixture.
...
def find_vsix(root: Path):
    """The newest `.vsix` `npm run package` left in the extension directory."""
    found = sorted((root / "vscode-extension").glob("*.vsix"))
    return found[-1] if found else None


def _run(argv, cwd: Path, core: Path) -> subprocess.CompletedProcess:
...
def _inside(path: Path, root: Path) -> bool:
    """`path` is `root` or lives under it. `Path.is_relative_to` is 3.9+, but
    this file also runs under whatever interpreter a validator happens to have."""
    try:
...
def check(vsix: Path, workdir: Path, problems) -> str:
    ...
    if not resolved or not _inside(Path(resolved), unpacked):
        problems.append(
    ...
    # 4. a real analysis, so the schema, the rules and the knowledge tables are used
