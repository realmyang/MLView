...
// `npm run pretest` and `npm run package` all run it first, so every path that
// ends in a packaged or a tested extension has a current core, and nobody has to
// remember `python tools/sync-core.py` before `vsce package`.
//
